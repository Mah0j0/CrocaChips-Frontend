...
const AppHeader: React.FC<AppHeaderProps> = ({userData}) => {
  ...
  const [filteredRoutes, setFilteredRoutes] = useState<ruta[]>([]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowDown") {
        ...
        event.preventDefault();
        handleRouteClick(filteredRoutes[highlightedIndex].path);
      }
    };
  ...
  }, [filteredRoutes, highlightedIndex]);
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (term.trim() === "") {
      ...
      const filtered = rutas.filter((route) =>
  ...
  // Redirigir al usuario al hacer clic en una opción
          ...
          <div className="hidden lg:block">
            <div className="relative">
              ...
              <input
                ref={inputRef}
                type="text"
                placeholder="Buscar Opciones de Menú"
                value={searchTerm}
                onChange={handleSearchChange}
                className="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-gray-900 dark:bg-white/[0.03] dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[430px]"
              />
              {filteredRoutes.length > 0 && (
                  <ul className="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
